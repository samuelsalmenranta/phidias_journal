...
export const STRATEGIES: StrategySpec[] = [
  ...
  mk({
    ...
    logic: "Iso kynttilä = käänne — iso vihreä = myy, iso punainen = osta",
    days: "Tiistai–perjantai (ei maanantaisin)",
    noMonday: true,
    ...
    warning:
      "HG cutoff: sulje viimeistään 16:45 ET / 23:45 Helsinki (DST: 22:45). Jos 16:30 ET entry on auki 16:43–16:44 ET, sulje manuaalisesti ennen Lucid 16:45 ET cutoffia.",
    notes: [
      "Sessio alkaa 13:30 ET / 20:30 Helsinki (DST: 19:30). Odota vähintään 2 kynttilää ennen kuin katsot signaalia.",
      "Signaaliehto: (|close − open|) / open × 100 ≥ 0.5 % JA kynttilän body ≥ 55 % koko kynttilän pituudesta.",
      "Vihreä signaali (close > open) → myy seuraavan kynttilän avauksessa.",
      "Punainen signaali (close < open) → osta seuraavan kynttilän avauksessa.",
      "Viimeinen sallittu entry: 16:15 ET kynttilä / 23:15 Helsinki (DST: 22:15).",
      "Stop 16 tickiä, target 72 tickiä, max hold 2 kynttilää.",
      "Tiistai–perjantai (ei maanantaisin).",
    ],
  }),
  mk({
    ...
    logic: "Jatka trendiä — 4 kynttilän liike ≥ 0.3 % = osta tai myy suuntaan",
    days: "Tiistai–perjantai (ei maanantaisin)",
    noMonday: true,
    sessionName: "Europe / Globex",
    sessionStartEt: "02:00",
    ...
    notes: [
      "Laske 4 kynttilän liike: (viimeisimmän kynttilän close − 4 kynttilää sitten olevan kynttilän open) / sen open × 100.",
      "Liikkeen oltava yli +0.3 % tai alle −0.3 %.",
      "Lisäehto: signaalikynttilän on suljettava liikkeen suuntaan (nousu → vihreä, lasku → punainen).",
      "Positiivinen liike → osta, negatiivinen → myy seuraavan kynttilän avauksessa.",
      "Entry mahdollinen: 03:00–04:45 ET / 10:00–11:45 Helsinki (DST: 09:00–10:45).",
      "Stop 48 tickiä, target 120 tickiä, max hold 4 kynttilää.",
      "Sulje viimeistään 05:00 ET / 12:00 Helsinki (DST: 11:00).",
      "Tiistai–perjantai (ei maanantaisin). 20 micros = 2 mini-equivalent.",
    ],
  }),
  mk({
    id: "GC_gap_phase1_16",
    ...
    logic: "Seuraa aamun gappia — gap ylös = osta, gap alas = myy",
    days: "Maanantai–perjantai (kaikki päivät)",
    noMonday: false,
    sessionName: "US open",
    ...
    warning:
      "Korkein EV mutta riskisin jalka. Seuraa 10 treidin rullaavaa PnL:ää tarkasti.",
    notes: [
      "Edellinen päivä pitää olla punainen (9:30 ET open > 16:45 ET close).",
      "Gap = (tämän päivän 09:30 open − edellisen päivän 16:45 close) / edellisen päivän 16:45 close × 100.",
      "Gap oltava yli +0.7 % (LONG) tai alle −0.7 % (SHORT).",
      "Vahvistus: 09:30–09:45 ET kynttilä sulkeuduttava gapin suuntaan (gap ylös → vihreä, gap alas → punainen).",
      "Entry: 09:45 ET kynttilän avaus / 16:45 Helsinki (DST: 15:45).",
      "Stop 24 tickiä, target 72 tickiä, max hold 4 kynttilää.",
      "Sulje viimeistään 16:45 ET / 23:45 Helsinki.",
      "Kaikki päivät maanantaista perjantaihin.",
    ],
  }),
];
